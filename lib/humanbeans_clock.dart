...
class _HumanbeansClockState extends State<HumanbeansClock>
  ...
  /// them form [Timer] callback if they're not playing will throw an exception
  ...
  ///
  /// It's implemented as a class to provide functionality to show random animation from the list depending on how much time the bird
  /// has to stay on the screen.
  late final BirdAnimations _birdAnimations = BirdAnimations();
  ...
  void didChangeDependencies() {
    ...
    _imagesFuture = Future.wait([
      _loadImage(AssetBundleImageKey(
          ...
          scale: _utils.scaleDimensions(1))),
      _loadImage(AssetBundleImageKey(
  ...
  void _birdAnimationListener(String name) {
    ...
    if (name == _birdAnimations.getFlyOut()) {
      ...
      final nextAnimationName = _birdAnimations.getRandom(
          ...
          55000
              ...
              +
              (_birdTime?.millisecondsSinceEpoch ?? 0)
      ...
      if (nextAnimationName != null) {
        ...
        _birdTimer = Timer(const Duration(seconds: 2),
  ...
  void _getBirdTime() async {
    ...
    try {
      ...
      final birdTimeMillis = int.parse(birdTime);
  ...
  Widget build(BuildContext context) {
    ...
    final accessibilityTime = DateFormat('jm').format(_dateTime);

    return
        // Return the [ClockUiInheritedModel] that holds all the data for the clock
        Semantics(
      label: 'Current time: $accessibilityTime}',
