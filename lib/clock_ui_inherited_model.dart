...
class ClockUiInheritedModel extends InheritedModel<String> {
  ...
  /// Static method to get hold of the widget from child context.
  static ClockUiInheritedModel of(BuildContext context, String aspect) {
    final model = InheritedModel.inheritFrom<ClockUiInheritedModel>(context,
        aspect: aspect);
    if (model == null) {
      throw FlutterError(
          'ClockUiInheritedModel.of(context) called from a context that did not contain it.');
    } else {
      return model;
    }
  }

  /// Check if leaf should update.
